...
def sepia(img_matrix):
    ...
    for y in range(height):
        new_new_list = []
        for x in range(width):
            new_number1 = int(img_matrix[y][x][0]*0.39+img_matrix[y][x][1]*0.75+img_matrix[y][x][2]*0.19)
            new_number2 = int(img_matrix[y][x][0]*0.35+img_matrix[y][x][1]*0.69+img_matrix[y][x][2]*0.17)
            new_number3 = int(img_matrix[y][x][0]*0.27+img_matrix[y][x][1]*0.53+img_matrix[y][x][2]*0.13)
            if new_number1 > 255:
                new_number1 = 255
            if new_number2 > 255:
                new_number2 = 255
            if new_number3 > 255:
                new_number3 = 255
            the_new_list = [new_number1,new_number2,new_number3]
            new_new_list.append(the_new_list)
...
def flip_horizontal(img_matrix):
    ...
    for y in range(height):
        ...
        for x in range(width-1,-1,-1):
            new_new_list.append(img_matrix[y][x])
        new_list.append(new_new_list)
    return new_list
    #TODO: Finish this function
...
def double_size(img_matrix):
    ...
    height = int(len(img_matrix)/2)  #Height = # of rows, i.e. length of matrix
    width = int(len(img_matrix[0])/2) #Width = # of columns, i.e. length of one row
    for y in range(height):
        for x in range(width):
            new_matrix[2*y][2*x]=img_matrix[y][x]
            new_matrix[2*y][2*x+1]=img_matrix[y][x]
            new_matrix[2*y+1][2*x]=img_matrix[y][x]
            new_matrix[2*y+1][2*x+1]=img_matrix[y][x]
    return new_matrix
    #TODO: Finish this function
